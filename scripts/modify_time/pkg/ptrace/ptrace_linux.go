...
func Trace(pid int) (*TracedProgram, error) {
	...
	for {
		...
		for _, thread := range threads {
			...
			if err != nil {
				log.Printf("attach failed, tid: %d, err: %v", tid, err)
				...
				if retryCount[tid] < threadRetryLimit {
					log.Printf("retry attaching thread, tid: %d, retry count: %d", tid, retryCount[tid])
					continue
				}
			...
			defer func() {
				if !traceSuccess {
					...
					if err != nil {
						if !strings.Contains(err.Error(), "no such process") {
							log.Fatal(err, "detach failed", "tid", tid)
						}
					}
				}
			}()
			...
			log.Printf("attach successfully, tid: %d", tid)
			tids[tid] = true
			tidMap[tid] = true
		}
...
func (p *TracedProgram) Detach() error {
	for _, tid := range p.tids {
		log.Printf("detaching thread, tid: %d", tid)
	...
	log.Printf("successfully detach and rerun process, pid: %d", p.pid)
	return nil
}
...
func (p *TracedProgram) FindSymbolInEntry(symbolName string, entry *mapreader.Entry) (uint64, uint64, error) {
	...
	log.Printf("load offset: %x", loadOffset)
	...
	// 	log.Printf("symbol %d: %+v", i, symbol)
	// }
