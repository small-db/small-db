...
/// changed due to another transaction.
#[test]
#[cfg(any(feature = "repeatable_read", feature = "serializable"))]
fn test_anomaly_phantom_2() {
    let table_pod = new_random_btree_table(2, 0, None, 0, TreeLayout::LastTwoEvenlyDistributed);
    let table = table_pod.rl();

    // init: insert 2 rows (key=10, key=20)
    {
        let tx = Transaction::new();
        insert_row(&table, &tx, 10);
        insert_row(&table, &tx, 20);
        tx.commit().unwrap();
    }
    ...
    {
        let mut iter = table.iter(&read_tx);
        let v = iter.next().unwrap().get_cell(0).get_int64().unwrap();
        assert!(v == 10);

        // write and commit a new row
        {
            let write_tx = Transaction::new();
            insert_row(&table, &write_tx, 11);
            write_tx.commit().unwrap();
        }
    ...
    // check again
    // {
    //     let iter = table.iter(&read_tx);
    //     assert!(iter.count() == init_count);
    // }
}
